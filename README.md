...
| **Extraction** | Reads and parses every page of the PDF into plain text, caches the result under `cache/` keyed by SHA-256 hash so an unchanged file is never re-parsed, then measures text coverage and retries with a second parser if the first yields poor results. | Check pages-with-text / total-pages ratio and compare extracted text against a manual copy. | `pypdfium2` (C extension, releases GIL) is tried first so the web server stays responsive during extraction; `pdfplumber` is used as a quality fallback. The cache never expires, so a corrected PDF must be manually cleared. |
...
**Memory management & concurrency:** `pypdfium2` (a C extension wrapping Google's PDFium library) is used as the primary PDF extractor because it releases the Python GIL during parsing, keeping the uvicorn event loop free to serve other requests while a PDF is being processed. `pdfplumber` is retained as a quality fallback. Job and upload state is stored as small JSON files under the system temp directory (rather than in-process memory) so the server remains stateless across restarts. A cleanup loop (every 5 minutes) automatically removes completed or errored jobs after 2 hours and stale upload sessions after 15 minutes, including their associated files on disk. The `Dockerfile` includes a `HEALTHCHECK` so container runtimes can detect and restart an unresponsive process.
...
2. **Extraction** — Loads from `cache/<sha8>_<stem>.json` (under the project root) when the PDF bytes match; otherwise extracts with `pypdfium2` (primary — C extension, releases Python GIL), measures quality (pages with sufficient text), retries with `pdfplumber` if quality is below threshold, then saves cache.
